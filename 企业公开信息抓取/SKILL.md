...
## work-analysis-loop 集成说明（v3.4.0 新增）

本 Skill 从 v3.4.0 起在 **`work-analysis-loop` 护栏**下执行，具体映射：

| 执行阶段 | 对应 Loop Round | 停止条件 |
|---------|---------------|---------|
| 第一阶段（步骤1-9）：资料采集+初稿 | Round 1-6 | 关键年报无法下载、API 连续3次失败 |
| 第二阶段（步骤10）：检查验证 | Round 7 | — |
| 第三阶段（步骤11）：修订V2 | Round 8-9 | — |
| 第四阶段（步骤12-13）：对比评估 | Round 10 | 用户确认最终版后停止 |

### 数据层级标注规范（强制）

所有从 PDF 提取的数据必须按以下层级标注：

| 层级 | 标注格式 | 用途 |
|------|---------|------|
| `[事实]` | 直接引用年报原文数字 | 任何科目金额 |
| `[抽数]` | 结构化后的时间序列 | extracted_data.json 中的数据 |
| `[计算]` | 由抽数计算的比率/指标 | 财务指标计算结果 |
| `[假设]` | 数据缺失时的替代假设 | 必须在报告中明确标注 |
| `[推断]` | 基于数据的分析结论 | 经营分析、风险评估 |
| `[观点]` | 审查判断 | 不含审查结论（由审查员写）|

### 缺失数据强制规则

- **绝对禁止**：将缺失科目当作 0 填入报告或 Excel
- 缺失科目必须标注：`[缺失：未披露]` / `[缺失：无法访问]` / `[缺失：无法核验]`
- 缺失比例 > 20% 的报表：在报告中显著标注数据局限性

### 资料索引生成（v3.4.0 新增）

每次完成任务后，在 `02-分析报告/` 下生成 `资料索引.md`：
...
| 编号 | 触发条件 | 优先级 |
|------|---------|---------|
| T1 | 用户说出"A股"/"港股"/"上市公司"+"年报"/"披露"/"审查报告" | P0 |
| T2 | 用户上传了PDF文件（年报/招股书/评级报告） | P0 |
| T3 | 用户提供了股票代码（6位数字/A股，4位数字/港股） | P1 |
| T4 | 用户说"IPO"/"招股书"/"聆讯" | P1 |
| T5 | 用户说"债券募集说明书"/"信用评级报告" | P1 |

排除条件：
- E1: 用户仅说"查一下XX公司"但未指定要公开披露资料 → 先询问需要哪些资料
- E2: 用户说的是"帮我做估值"→ 触发企业估值分析Skill

触发后动作：
1. 识别企业类型（A股/港股/IPO/债券发行主体）
2. 输出资料清单（参考「基础资料清单」章节）
3. 请用户确认或补充资料
4. 创建 `.state.json`，开始四阶段执行

---

## State 持久化（断点恢复）

本 Skill 采用四阶段迭代（收集→提取→交付V1→交付V2），每阶段结束后写入 `.state.json`。

### State 字段（本 Skill 特定扩展）

| 字段 | 类型 | 说明 |
|-------|------|------|
| `delivery_version` | string | 当前交付版本：V1/V2/FINAL |
| `stages_completed` | list[string] | 已完成阶段：["COLLECTING", "EXTRACTING", "DELIVERING_V1"] |
| `stages_pending` | list[string] | 待执行阶段 |
| `verification_results` | object | 各阶段验证结果 |
| `current_file` | string | 当前正在处理的文件名 |
| `extraction_progress` | string | 提取进度：已提取科目数/总科目数 |

### 四阶段 State 示例

```json
{
  "task_id": "EPD-2026-0628-001",
  "skill": "企业公开信息抓取",
  "version": "3.5.0",
  "stage": "DELIVERING_V2",
  "iteration": 4,
  "status": "RUNNING",
  "stages_completed": ["COLLECTING", "EXTRACTING", "DELIVERING_V1"],
  "stages_pending": ["DELIVERING_V2", "COMPARING", "FINALIZING"],
  "delivery_version": "V2",
  "verification_results": {
    "V1": {"status": "pass", "warnings": 2},
    "V2": {"status": "pending", "warnings": null}
  },
  "evidence_index": {
    "年报2023": ["P45", "P52", "P88"],
    "季报2024Q1": ["P12"]
  ...
  "data_gaps": [
    {"field": "2023年Q3经营现金流", "reason": "未披露", "action": "标注为缺失"}
  ],
...
启动 Skill 时：
  if .state.json 存在：
    ...
    → 输出："检测到未完成任务 {task_id}，已完成阶段：{stages_completed}，是否继续？"
    → 用户确认 → 从 stages_pending[0] 继续执行
    → 用户拒绝 → 备份 .state.json 为 .state.json.archived，从头开始
  else：
    → 从头开始，生成新 task_id（格式：EPD-MMDD-XXX）
```
...
```json
{
  ...
  "skill": "企业公开信息抓取",
  "skill_version": "3.5.0",
  "stop_reason": "S1: 四阶段交付完成",
  "state_snapshot": {
    "stage": "COMPLETED",
    "stages_completed": ["COLLECTING", "EXTRACTING", "DELIVERING_V1", "DELIVERING_V2"],
    "delivery_version": "V2"
  },
  "deliverables": [
    {"name": "财务数据底稿", "path": "01-底稿/财务数据底稿.xlsx", "status": "completed"},
    {"name": "授信审查报告", "path": "04-报告/授信审查报告.docx", "status": "completed"},
    {"name": "资料索引", "path": "00-工作区/资料索引.md", "status": "completed"}
  ],
  "evidence_summary": {
    "total_sources": 5,
...
- 阶段：`COLLECTING → EXTRACTING → DELIVERING_V1 → DELIVERING_V2 → COMPLETED`
- 迭代轮次：`4`
- 交付版本：`V2`
- 数据完整度：`92%`
...
| 文件 | 路径 | 状态 |
|------|------|------|
| 财务底稿 | `01-底稿/财务数据底稿.xlsx` | ✅ 完成 |
| 授信报告 | `04-报告/授信审查报告.docx` | ✅ 完成（V2） |
| 资料索引 | `00-工作区/资料索引.md` | ✅ 完成 |
...
1. 2023年Q3季报未获取，经营现金流数据标注为「缺失：未披露」
   → 建议：如有季报，可补充提取
...
1. 如需更新数据 → 重新运行，传入2024年年报PDF
2. 如需补充分析 → 传 `credit-review-workbench` 继续授信审查
...
| 3.4.0 | 2026-06-28 | **护栏集成**：①新增 work-analysis-loop 护栏集成映射表；②新增数据层级标注规范（六层分级强制要求）；③缺失数据强制规则（禁止当作0处理）；④新增资料索引生成模板（每次任务结束后自动生成溯源清单）；⑤与 credit-review-workbench 建立双向路由关系 |
| 3.5.0 | 2026-06-28 | **loop-engineering 改造**：①frontmatter 增加 loop_engineered 标记；②新增 Trigger（触发条件）章节；③新增 State 持久化（断点恢复）章节；④新增 Handoff 格式（交接包）章节；⑤采用统一 State Schema |
